...
- `libopencv.so` 全图像管线的剩余算子（找色/灰度/裁剪/缩放/旋转/特征）；模板匹配 + `decode`/`release` 已随 §9.2 落地；MediaProjection 会话式截屏/录屏仍待（换 producer 即插）。
