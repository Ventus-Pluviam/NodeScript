...
4. **下一步**（按可执行性排序）：(a) 等设备的那笔账——§7.7 实测数字 + exec/dlopen / findColor 红测；(b) ICU **已闭环**：旗标改 + Actions 重编 success + 体积差已回填 §15（`libnode.so` +10.70 MiB），仅剩 zh/en 的 `Intl.*` 运行期实测归真机那批；(c) §14 P0 剩余项按 §12.2 接线现状表逐条核。**第 10 项（脚本库/编辑器页、npm 呈现层）2026-09-26 拍板暂不排期**。
