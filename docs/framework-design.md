...
>
> 仍待 CI/后续轨：jniLibs 交付（`libnoden.so`/`libnode.so`/addon `.node` 落位与
> exec 红测）、`libc++_shared.so`（libnode 链）装载与真机联调。
...
- 长跑脚本自身需要**WakeLock** 时用 `power_manager`（引擎进程请求 → `:main` 对应 FGS 加唤醒锁的 acquire/release，配套超时自动释放）——**仍是 P1**（命名空间未实现）；本期 `WakeLockLedger` 的 `hold(token, timeoutMillis)` 就是照这个形状留的插口，届时引擎侧请求直接进同一本账，不必改账本语义。
- **P0 已落地（`:main` 侧 FGS + 真唤醒锁，2026-09-23）**：三个可分离的缝，判断全在 JVM 可测面，系统接触面各收在一个类里。
  - **`WakeLockOps` / `WakeLockLedger`**（`app/.../shell/WakeLock.kt`）：`AndroidWakeLockOps` = 真 `PARTIAL_WAKE_LOCK`（`setReferenceCounted(false)`，acquire/release 异常一律吞成 `false` + 日志）；`WakeLockLedger` = **token 引用计数 + 超时自动释放**——`hold(token, timeoutMillis)` 只在**首次**（账本为空）时真取锁，**取锁失败不记账**（"记了账却没锁"是假绿之源）；`release(token)` 用"先查存在再删"（超时 token 的值为 null，照样能释放）；`sweep()` 释放到期项并返回名单；`isHeld()` = **账本非空 ∧ `ops.held`**——两侧任一说"没有"就一律算没锁。token/超时的形状就是给 P1 `power_manager` 预留的插口（引擎进程请求 → FGS 加锁走同一账本），本期不实现命名空间。
