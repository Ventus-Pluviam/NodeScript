...
- `libopencv.so` 全图像管线的剩余算子（灰度/裁剪/缩放/旋转/特征）；**找色已落地**（2026-09-25，§9.2：单色 + 逐分量容差 + 可选区域 + 首个命中，四层同改，`x=-1` 哨兵与“扫过 0 像素”两条口径），模板匹配 + `decode`/`release` 亦已随 §9.2 落地；MediaProjection 会话式截屏/录屏仍待（换 producer 即插）。
