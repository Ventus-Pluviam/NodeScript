...
§12.2 是**命名空间清单**（有什么、接线到哪、谁注入），§12.3 是**调用形状**（怎么调、回什么、哪里会抛）。两者冲突时以本节为准（本节是实测），并应回来改 §12.2。§18 的**待拍板缺口**别在本节自行发明口径：第 8 项（截屏帧 ↔ images 帧的通路）**已于 2026-09-26 落地**（(b) 两缝共用帧表，见 §9.2），余下第 9 项（`images.decode` 的相对路径口径 —— 四层都不解析路径，相对写法按 `:main` 的 CWD（= `/`）解析，`fromFile('part.png')` 回 `ERR_FILE_NOT_FOUND` 而报的路径是对的，看起来像"文件真的不在"。**口径拍板前，本节示例一律写绝对路径**）。
