...
> 烟测逐字比对；裸嵌对象会被宿主扁平解码整帧拒掉）。仍待 CI/后续轨：jniLibs 交付
> （`libnoden.so`/`libnode.so` 落位与 exec 红测）、`:main` 桥监听
> （`LocalServerSocket(String)` abstract + uid 门禁）、addon 的 JS 消费面
> （`setup(onFrame)` 结算）、`libc++_shared.so`（libnode 链）装载。
...
3. `dlsym _ZN4node5StartEiPPc` → `node::Start` 单 isolate/context，argv =
   ...
   JS 侧 `setup(onFrame)` 建 data TSF 由 facade 接入时调（P0 未接则响应帧计入
...
至此 §8.4 三路判据、采样、调度、心跳打点全部闭环。**pid 半边已接线**：`NodeProcessEngine`（`:engine:node-process`，Kotlin spawn）的 `EngineRunReceipt.pid` = spawn 瞬间真子进程 pid（`NodeProcessEngineRealSpawnTest` 实测 >0 且非自身；`:app` E2E 经 `watchAnchors` 锚点 pid>0 复验），看门狗 `/proc` 采样锚点在 spawn 路径上是活的。**心跳的生产链已备**：spawn env 下传 `AUTOSCRIPT_RUN_ID` → kBootstrap 500ms 自动打点 / 桌面脚本显式 `startHeartbeat`，E2E 实测 `heartbeatMillis(runId)` 落账非 null；但**设备面仍受两道约束**——`:main` 桥监听（abstract `LocalServerSocket`）与 addon 的 JS 消费面未落。在这两道接上之前，真机上当前生效的是「未 spawn / 宿主不给 pid → noPid」「宿主不打点 → noHeartbeat」两条量不到路径 —— 这由 `Tick` 的三个清单如实区分，不是一个笼统的 `unmeasurable`。
