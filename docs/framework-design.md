...
> `hostSocketName`、绑定失败 = 离线降级不注入（不触发 exit 3）。仍待 CI/后续轨：
> jniLibs 交付（`libnoden.so`/`libnode.so` 落位与 exec 红测）、addon 的 JS 消费面
