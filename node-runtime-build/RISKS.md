...
## 3. ICU：已改 `small-icu`（zh,en）—— 体积已量（中）

**已拍板（2026-09-26，§18 第 4 项）**：只要中文 + 英文。构建参数由
`--with-intl=none` 改为 `--with-intl=small-icu --with-icu-locales=zh,en`。

- **不联网**：`deps/icu-small/` 带 `README-FULL-ICU.txt` → `configure.py` 走
  `canned_is_full` 分支直接用仓内 ICU 源（`tools/icu/current_ver.dep` 钉 78.3），
  只有在仓内缺失/被裁时才会回退到下载 icu4c —— 构建风险从"下载失败"降为"无"。
- `root` locale 由 configure 自动并入，故 `zh,en` 是**除 root 外**的全部。
- 语言覆盖：zh + en 之外（如 `ja_JP`）按 en 回退，不静默给空结果。
- **体积已实测（2026-09-26，两个 `node-slice` artifact 对比）**：
  `libnode.so` **70,725,976 → 81,950,376 B（+11,224,400 B = +10.70 MiB = +15.87%）**。
  基线 run `36153816811`（`intl=none`，`config.gypi` 里 `icu_small=false`）vs
  新 run `36185853302`（success，`icu_small=true` / `icu_locales=en,root,zh` /
  `icu_path=deps/icu-small` / `icu_ver_major=78`）。旧估数 "~10MB+" 是 small-icu
...
- **剩余欠账 = 运行期**：`Intl.DateTimeFormat` / `Intl.Collator` 在 zh/en 上的真机
  ...
  非 zh/en locale 回落 en 是明口径不是 bug。

历史（回读用）：无 ICU 时 `Intl.*`/`toLocaleString` 受限，不影响 npm 安装语义；
