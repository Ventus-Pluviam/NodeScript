...
- **kleidicv 是软降级，必须留审计行，且 OFF 的原因别猜**：`ocv_download` 失败只 WARNING 不 fatal，产物可能悄悄从"带 kleidicv 加速"变成"纯 OpenCV"而**功能不报错、体积缩小**。对策：pin 写死在 `VERSIONS.env`（`KLEIDICV_COMMIT=26.03` + md5），构建时 grep 上游 `hal/kleidicv/kleidicv.cmake` 的两处 pin（上游换 pin 即 die），收尾把 `HAVE_KLEIDICV` 的 ON/OFF 与**三个可判读事实**（cache 值 / `3rdparty/kleidicv/kleidicv-<pin>` 源码是否在场 / `libkleidicv.a` 是否生成）一起写进审计行。**2026-09-24 教训**：首次跑审计行是 OFF，我把原因说成"gitlab.arm.com 网络不可达"——**这是未经验证的猜测且是错的**。日志实证 tarball 拉下来了（同 pin md5 与 `VERSIONS.env` 一致）、125 个 `.cpp` 照编、6 个 target 全 `Built`，只有 `HAVE_KLEIDICV` 是 OFF。所以审计行必须能把"下载失败"（源码目录空）和"下了但 OpenCV 没认"（源码在、cache 非 ON，下一步查 `hal/kleidicv/kleidicv.cmake` 的判定条件）这两种情况分开；在没读到 cargo/源码证据前，不要替编译器宣布归因。
