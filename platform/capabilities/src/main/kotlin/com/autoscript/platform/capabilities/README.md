...
| 图像面宿主机语义门禁 | `bridge/image/test/cpp/run-host-tests.sh`（OpenCV 4.14.0 同 commit x86_64 静态库直链 `imgnative.cpp`；`host_color_test` 36 例 + `host_decode_norm_test` 21 例 + `host_match_test` 24 例 + `host_gray_test` 28 例 + `host_crop_test` 46 例 + `host_resize_test` 45 例 + `host_rotate_test` 45 例 = 245 例，decode 归一/findColor/matchTemplate/灰度/裁剪/缩放/旋转七个算子的判读都有；特征仍是缺口） | 不在这份 README 的门禁范围（它是 C++ 面、住 `:bridge:image`）：补在这儿只为「图像面有第三道门」这件事有据可查。NDK `-fsyntax-only` 只管 aarch64 能编、它管判读对，两者互不替代（2026-09-25 它抓出 `IMREAD_COLOR` 丢 alpha 导致 findColor 的 a 分量从未参与判定）|
...
`images` native 面的剩余算子（特征；灰度、裁剪、缩放与旋转已落计算核，桥面刻意未开 —— §9.2：桥面五方法与
`:domain` `ImageAnalyzer` SPI 已就位，`libopencv.so` 的 `decode`/`matchTemplate`/`findColor`
也已接，缺的是那些还没开桥面的操作）。计算核判读的宿主机门禁已补上（`run-host-tests.sh`，245 例，覆盖 decode 归一/findColor/matchTemplate/灰度/裁剪/缩放/旋转）—— 新算子落地时**先补它的 host 断言再上真机**，否则又是一次「三门全绿、alpha 从来没参与判定」。
