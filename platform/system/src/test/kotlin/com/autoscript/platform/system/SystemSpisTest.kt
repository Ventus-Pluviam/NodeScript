...
/**
 * 实现入口的形状测试（docs §12.2）：[SystemSpis.Bundle] 的八件都**声明成 `:domain` 契约类型**。
 *
 ...
 *    （datastore/zip/settings 2026-09-22 落地后从四件变七件，notification 同日补到八件；dialogs 仍不许出现在名单里）。
 */
class SystemSpisTest {

    @Test
    fun `Bundle 八件都是 domain 契约类型（具体类不外泄）`() {
    ...
    fun `dialogs 不在束里：缺位就是缺位，不拿假实现凑`() {
        ...
        assertEquals(
            setOf("shell", "device", "app", "floatingWindow", "datastore", "zip", "settings", "notification"),
            names,
        )
    }
