...
exports.power = {
     ...
     */
    async acquire(timeoutMillis, opts = {}) {
        const r = (await runtime_1.runtimeBridge.invoke('power_manager', 'acquire', { timeoutMillis }, { ttl: opts.timeout ?? 5_000 }));
        return r.token;
    },
    /**
     * 放自己那一份；false = 该 token 当时并未持有（重复放/陌生 token/已过期）。
     */
    async release(token, opts = {}) {
        return (await runtime_1.runtimeBridge.invoke('power_manager', 'release', { token }, { ttl: opts.timeout ?? 5_000 })) === true;
    },
    /** 锁现状：held（门禁判据）+ holders（账本席位数）。 */
    async status(opts = {}) {
        return (await runtime_1.runtimeBridge.invoke('power_manager', 'status', null, { ttl: opts.timeout ?? 5_000 }));
    },
};
