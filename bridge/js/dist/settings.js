...
exports.settings = {
    ...
    async canWrite(opts = {}) {
        return (await runtime_1.runtimeBridge.invoke('settings', 'canWrite', null, { ttl: opts.timeout ?? 5_000 })) === true;
    },
    /** 读字符串设置；键缺失 → `null`（绝不拿空串冒充缺失）。 */
    async getString(key, opts = {}) {
        return (await runtime_1.runtimeBridge.invoke('settings', 'getString', { key }, { ttl: opts.timeout ?? 5_000 }));
    },
    /** 读整型设置；键缺失 → `null`（0 是合法亮度，不拿 0 冒充）。 */
    async getInt(key, opts = {}) {
        return (await runtime_1.runtimeBridge.invoke('settings', 'getInt', { key }, { ttl: opts.timeout ?? 5_000 }));
    },
    /** 写字符串设置（空串是合法值）。未授 `WRITE_SETTINGS` → 抛 `ERR_PERMISSION_DENIED`。 */
    async putString(key, value, opts = {}) {
        await runtime_1.runtimeBridge.invoke('settings', 'putString', { key, value }, { ttl: opts.timeout ?? 5_000 });
    },
    /** 写整型设置。失败口径同 putString（未授 `ERR_PERMISSION_DENIED`、已授权仍被拒 `ERR_IO`）。 */
    async putInt(key, value, opts = {}) {
        await runtime_1.runtimeBridge.invoke('settings', 'putInt', { key, value }, { ttl: opts.timeout ?? 5_000 });
    },
};
