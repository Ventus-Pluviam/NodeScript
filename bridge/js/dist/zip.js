...
exports.zip = {
    ...
    async compress(source, archive, opts = {}) {
        await runtime_1.runtimeBridge.invoke('zip', 'compress', { source, archive }, { ttl: opts.timeout ?? 60_000 });
    },
    /** 解压 zip → 目标目录（不存在则创建；zip-slip 越界条目在宿主侧整次拒绝）。 */
    async extract(archive, targetDir, opts = {}) {
        await runtime_1.runtimeBridge.invoke('zip', 'extract', { archive, targetDir }, { ttl: opts.timeout ?? 60_000 });
    },
};
