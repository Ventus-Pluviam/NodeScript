...
exports.notification = {
    ...
    async canPost(opts = {}) {
        return (await runtime_1.runtimeBridge.invoke('notification', 'canPost', null, { ttl: opts.timeout ?? 5_000 })) === true;
    },
    /** 发/覆盖一条通知。未授权 → 抛 `ERR_PERMISSION_DENIED`。 */
    async post(spec, opts = {}) {
        const payload = { id: spec.id, text: spec.text };
        if (spec.title !== undefined && spec.title !== null)
            payload.title = spec.title;
        await runtime_1.runtimeBridge.invoke('notification', 'post', payload, { ttl: opts.timeout ?? 5_000 });
    },
    /** 按 id 撤销；幂等无回执（撤一个没发过的 id 什么也不会发生）。 */
    async cancel(id, opts = {}) {
        await runtime_1.runtimeBridge.invoke('notification', 'cancel', { id }, { ttl: opts.timeout ?? 5_000 });
    },
};
