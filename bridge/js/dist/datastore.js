...
exports.datastore = {
    ...
    async get(key, opts = {}) {
        const r = await runtime_1.runtimeBridge.invoke('datastore', 'get', { key }, { ttl: opts.timeout ?? 5_000 });
        return r?.found ? r.value : undefined;
    },
    /** 写/覆盖一键（任意 JSON 可序列化值）。 */
    async put(key, value, opts = {}) {
        await runtime_1.runtimeBridge.invoke('datastore', 'put', { key, value }, { ttl: opts.timeout ?? 5_000 });
    },
    /** 删一键；回是否真的移除了东西（键不存在 → false，幂等不抛）。 */
    async remove(key, opts = {}) {
        return (await runtime_1.runtimeBridge.invoke('datastore', 'remove', { key }, { ttl: opts.timeout ?? 5_000 })) === true;
    },
    async contains(key, opts = {}) {
        return (await runtime_1.runtimeBridge.invoke('datastore', 'contains', { key }, { ttl: opts.timeout ?? 5_000 })) === true;
    },
    /** 全部键（顺序不作契约保证，调用方如需稳定序自行排序）。 */
    async keys(opts = {}) {
        return (await runtime_1.runtimeBridge.invoke('datastore', 'keys', null, { ttl: opts.timeout ?? 5_000 }));
    },
    async clear(opts = {}) {
        await runtime_1.runtimeBridge.invoke('datastore', 'clear', null, { ttl: opts.timeout ?? 5_000 });
    },
};
