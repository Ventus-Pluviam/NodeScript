...
export const power = {
   ...
   */
  async acquire(timeoutMillis: number, opts: { timeout?: number } = {}): Promise<string> {
    const r = (await runtimeBridge.invoke(
      'power_manager', 'acquire', { timeoutMillis }, { ttl: opts.timeout ?? 5_000 },
    )) as { token: string }
    return r.token
  },
   ...
   */
  async release(token: string, opts: { timeout?: number } = {}): Promise<boolean> {
    return (await runtimeBridge.invoke(
      'power_manager', 'release', { token }, { ttl: opts.timeout ?? 5_000 },
    )) === true
  },

  /** 锁现状：held（门禁判据）+ holders（账本席位数）。 */
  async status(opts: { timeout?: number } = {}): Promise<{ held: boolean; holders: number }> {
    return (await runtimeBridge.invoke(
      'power_manager', 'status', null, { ttl: opts.timeout ?? 5_000 },
    )) as { held: boolean; holders: number }
  },
}
