...
export const clipboard = {
  ...
  async getText(opts: { timeout?: number } = {}): Promise<string | null> {
    return (await runtimeBridge.invoke(
      'clipboard', 'getText', null, { ttl: opts.timeout ?? 5_000 },
    )) as string | null
  },

  /** 写剪贴板文本（空串是合法内容，原样存）。 */
  async setText(text: string, opts: { timeout?: number } = {}): Promise<void> {
    await runtimeBridge.invoke('clipboard', 'setText', { text }, { ttl: opts.timeout ?? 5_000 })
  },
}
